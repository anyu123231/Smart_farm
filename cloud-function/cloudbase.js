...
exports.main = async (event, context) => {
	try {
		...
		// 解析请求参数（支持多种调用方式）
		// SDK 调用：event.path 或 event.__route__
		// HTTP 调用：event.path 或 event.requestContext.path
		const path = event.path || event.__route__ || ''
		const httpMethod = event.httpMethod || event.httpMethod || 'GET'
		const queryString = event.queryString || event.query || {}
